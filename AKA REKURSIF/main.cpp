...
void findEvenNumbers(int N, int A[], int index, bool &foundEven) {
    // Basis rekursif: jika sudah mencapai akhir array
    if (index == N) {
        if (foundEven == false) {
            cout << -1;
        }
        ...
        findEvenNumbers(N, A, index+1, foundEven);
    }
}
...
int main() {
    ...
    // Memulai rekursi dari index 0
    findEvenNumbers(N, A, 0, foundEven);
